...
public class UploadProcessJob {
	...
	UploadProcessJob(String remoteHost, int ftpport, File localFile,
			String remoteFolder, String username, String pass, String uploadName)
		...
		uploadThread = new Thread(new Runnable() {
			public void run() {
				try {
					...
					ftpcom.transferData(remoteFolder, uploadName,
							localFile.getPath(), monitor);
	...
	public void UpdateUI() {
		...
		dlg.percent
				.setText((new StringBuilder(
						String.valueOf((int) ((float) (100L * (endSize)) / (float) totalbytes))))
						.append("%").toString());
		String filepath = localFile.getPath();
		int len = filepath.length();
		filepath = "当前上传:"
				+ (filepath.length() > 30 ? filepath.substring(0, 10) + "..."
						+ filepath.substring(len - 10, len) : filepath);
		dlg.curFilepath.setText(filepath);
		dlg.getProcessBar().setValue(
				(int) ((float) (10000L * endSize) / (float) totalbytes));
		...
		if (timepast != 0L)
			dlg.avgSpeed
					.setText(new StringBuilder(
							String.valueOf(shortSize((long) ((float) 1000
									* donebytes / (float) (timepast)))))
							.append("/秒").toString());
		if (donebytes != 0L) {
			/**
			 * 估计剩余的时间
	...
	private String getCurTime(long psec) {
		long sec = psec / 1000L;
		if (sec < 60)
			return new StringBuilder(String.valueOf(sec)).append("秒")
					.toString();
		else if (sec < 60 * 60 && sec >= 60)
			return new StringBuilder(String.valueOf(sec / 60)).append("分")
					.append(String.valueOf(sec % 60)).append("秒").toString();
		else {
			int hour = (int) sec / (60 * 60);
			int temp = (int) sec % (60 * 60);
			int min = temp / 60;
			int second = temp % 60;
			return new StringBuilder(String.valueOf(hour)).append("小时")
					.append(String.valueOf(min)).append("分")
